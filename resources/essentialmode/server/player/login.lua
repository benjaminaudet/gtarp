...
function LoadUser(identifier, source, new)
	...
	local result = MySQL:getResults(executed_query, {'permission_level', 'money', 'identifier', 'group', 'position'}, "identifier")

	local group = groups[result[1].group]
	Users[source] = Player(source, result[1].permission_level, result[1].money, result[1].identifier, group, position)
...
function registerUser(identifier, source, playerPed)
	if not hasAccount(identifier) then
		...
		MySQL:executeQuery("INSERT INTO users (`identifier`, `permission_level`, `money`, `group`, `position`, `ped`) VALUES ('@username', '0', '@money', 'user', '{"x":0, "y":0, "z":0}')",

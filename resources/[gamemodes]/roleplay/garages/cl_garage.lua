...
local garage = {
	...
	menu = {
		...
		["main"] = {
			...
			name = "main",
			buttons = {
				{name = "Rentrer son véhicule", description = "", action = "rentrer"},
				{name = "Sortir son véhicule", description = "", action = "sortir"},
			}
    	},
  	}
}
	...
	if bool and #garage_blips == 0 then
			...
			while #garage_blips > 0 do
				...
				for i,b in ipairs(garage_blips) do
					...
					if GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 2 then
  						DisplayHelpText("Appuyer sur ~INPUT_CONTEXT~ pour accéder au menu du garage")
						currentlocation = b
						inrange = true
					end
				end
		...
		Citizen.CreateThread(function()
			while #garage_blips > 0 do
				...
				local inrange = true
				for i,b in ipairs(garage_blips) do
					DrawMarker(1,b.pos.outside[1], b.pos.outside[2],b.pos.outside[3],0, 0, 0, 0, 0, 0, 5.001, 5.0001, 0.5001, 0,155, 255, 200, 0, 0, 0, 0)
					if GetDistanceBetweenCoords(b.pos.outside[1],b.pos.outside[2],b.pos.outside[3],GetEntityCoords(LocalPed())) < 4 then
						DisplayHelpText('Entrées et sorties de véhicules, ne pas encombrer inutilement.',0,1,0.5,0.8,0.6,255,255,255,255)
						currentlocation = b
						inrange = true
					end
				end
...
local backlock = false
	...
	while true do
		Citizen.Wait(0)
			if IsControlJustPressed(1,Keys['E']) and IsPlayerInRangeOfGarage() then
...
function ButtonSelected(button)
	...
	if this == "main" then
		if btn == "Rentrer son véhicule" then
			TriggerServerEvent('garages:CheckForVeh',source)
		elseif btn == "Sortir son véhicule" then
